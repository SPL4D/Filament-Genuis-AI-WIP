...
import { Send, Bot, User, Loader2, Sparkles, Save } from 'lucide-react';
import ReactMarkdown from 'react-markdown';

interface Props {
  isLoggedIn: boolean;
  onSaveProject: (title: string, history: ChatMessage[]) => void;
}

const ChatInterface: React.FC<Props> = ({ isLoggedIn, onSaveProject }) => {
  ...
  const handleSaveClick = () => {
    if (projectTitle.trim()) {
      onSaveProject(projectTitle, messages);
  ...
  return (
    <>
      <div className="flex flex-col h-[calc(100vh-64px)] md:h-[600px] w-full max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden my-4 md:my-8 relative">
        {/* Chat Header */}
        <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
            <div>
               <span className="font-semibold text-slate-700 block leading-tight">Conversational Mode</span>
          ...
          {isLoggedIn ? (
            <button 
              ...
              className="text-sm flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-600 hover:text-brand-600 hover:border-brand-200 transition-colors shadow-sm"
            >
              <Save size={16} /> Save
            </button>
          ) : (
            <span className="text-xs text-slate-400 bg-slate-100 px-2 py-1 rounded-md">Sign in to save</span>
          )}
        </div>
      ...
      {showSaveModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl animate-fade-in">
            <h3 className="text-lg font-bold mb-4 text-slate-900">Save Conversation</h3>
            <p className="text-slate-500 text-sm mb-4">Give your chat session a name to find it later in your dashboard.</p>
            <input 
              type="text" 
              autoFocus
              placeholder="e.g. Cosplay Armor Brainstorm"
